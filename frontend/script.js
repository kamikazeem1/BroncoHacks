...
function setupFormHandlers() {
  ...
  if (textBtn) {
    textBtn.addEventListener('click', async () => {
      ...
      const phoneInput = document.querySelector('#text-section input').value;
...
function displaySummary(data) {
  ...
  resultsContent.innerHTML = `
    <h3>So how <strong style="color: hsl(184, 79%, 49%);">fraudulent</strong> is this? Here's your <strong style="color: hsl(184, 79%, 49%);">summary</strong>:</h3>
    <ul>
      ${summary.safety_points.map(point => `<li>${point}</li>`).join('')}
    </ul>
    <p><strong style="color: hsl(184, 79%, 49%);">Safety Score</strong>: ${summary.safety_score}/100</p>
    <p><strong style="color: hsl(184, 79%, 49%);">Category</strong>: ${summary.safety_category}</p>
  `;
}
